...
def display_molecule_ascii(smiles: str):
    ...
    try:
        ...
        print(f"{Colors.BLUE}分子式:{Colors.ENDC} {Chem.rdMolDescriptors.CalcMolFormula(mol)}")
        print(f"{Colors.BLUE}分子量:{Colors.ENDC} {round(Chem.Descriptors.MolWt(mol), 2)}")
        print(f"{Colors.BLUE}氢键受体:{Colors.ENDC} {Chem.Lipinski.NumHAcceptors(mol)}")
        print(f"{Colors.BLUE}氢键供体:{Colors.ENDC} {Chem.Lipinski.NumHDonors(mol)}")
        print(f"{Colors.BLUE}旋转键:{Colors.ENDC} {Chem.Lipinski.NumRotatableBonds(mol)}")
...
def generate_docking_image(protein_path: str, ligand_smiles: str, pocket_center: tuple,
    ...
    """生成蛋白质-配体对接可视化图像 - 修复get_ipython问题"""
    try:
        if not HAS_PY3DMOL:
            print_warning("未安装py3Dmol，无法生成对接可视化。请使用pip安装py3Dmol。")
            return None

        # 修复: 正确地检查是否在Jupyter环境中
        in_jupyter = False
        try:
        ...
        if not in_jupyter:
            print_warning("不在Jupyter环境中，无法显示交互式3D可视化。将只生成静态信息文件。")
        ...
        if not in_jupyter:
            ...
            with open(output_path, 'w') as f:
                ...
                f.write("注意: 在非Jupyter环境下无法生成3D对接可视化。\n")
        ...
        # 保存图像
        if output_path is None:
            output_path = TMP_DIR / f"docking_{int(time.time())}.png"
