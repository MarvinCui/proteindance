...
class DrugDiscoveryAPI:
    ...
    @staticmethod
    def visualize_docking_results(protein_file: str, ligand_file: str, output_dir: str) -> Dict:
        """
        可视化对接结果
        """
        try:
            RequestValidator.validate_docking_visualization_request({
            ...
            api = DrugDiscoveryAPI()
            results = api.visualization_engine.generate_docking_visualization(
                protein_file=protein_file,
                ligand_file=ligand_file,
                output_dir=output_dir,
                use_pymol=True,
            ...
            logger.error(f"对接可视化失败: {str(e)}", exc_info=True)
            return {"success": False, "error": f"对接可视化失败: {e}"}
