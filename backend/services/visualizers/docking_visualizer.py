        ...
        def initialize_pymol():
            try:
                if not gui:
                    ...
                    os.environ['DISPLAY'] = ''
                    # 无头模式，不显示GUI
                    print("Initializing PyMOL in headless mode...")
                    pymol.finish_launching(['pymol', '-c', '-q'])  # 添加 -q 静默模式
                else:
                    # GUI模式
                    print("Initializing PyMOL in GUI mode...")
                    pymol.finish_launching()
                print("PyMOL initialization completed")
            except Exception as e:
                print(f"PyMOL initialization failed: {e}")
                raise
        
        # 使用线程初始化PyMOL，添加超时保护
        init_thread = threading.Thread(target=initialize_pymol)
        init_thread.daemon = True
        init_thread.start()
        init_thread.join(timeout=15)  # 15秒超时
        
        if init_thread.is_alive():
            raise RuntimeError("PyMOL initialization timeout - switching to headless mode")
            
        # 确保PyMOL命令可用
        if not hasattr(cmd, 'load'):
            raise RuntimeError("PyMOL commands not available after initialization")
    ...
    def run(self, create_movie=False):
        ...
        try:
            ...
            if self.gui:
                ...
                print("完成后请关闭PyMOL窗口")
                # The following line is for interactive sessions and might not work in all environments
                # It's a placeholder to keep the script alive for the GUI
                if hasattr(pymol.cmd, '_cmd') and hasattr(pymol.cmd._cmd, 'pygame'):
            ...
            print(f"错误: {e}")
            cmd.quit()
            # sys.exit(1) # Avoid exiting the whole process if used as a library
            return False
...
def visualize_docking_results(protein_file, ligand_file, output_dir="./docking_results", 
    ...
    if use_web:
        ...
        try:
            ...
            web_viz.set_files(protein_file, ligand_file, output_dir)
    ...
    if success:
        print(f"\n✓ Visualization completed! Results in: {output_dir}")
    else:
