...
class DiseaseRequest(BaseModel):
    disease: str
    innovation_level: Optional[int] = 5
...
async def get_disease_targets(req: DiseaseRequest):
    try:
        targets = AIService.get_disease_targets(
            req.disease, 
            req.innovation_level
        )
...
async def get_uniprot_entries(req: UniprotRequest):
    try:
        return AIService.get_uniprot_entries(req.gene_symbol)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_structure_sources(req: StructureRequest):
    ...
    try:
        ...
        af_path: Optional[Path] = AIService.download_alphafold(
            req.uniprot_acc, dest_dir=settings.TMP_DIR
        )

        # 查询该 accession 对应的 PDB 列表
        pdb_ids: List[str] = AIService.get_pdb_ids_for_uniprot(req.uniprot_acc)
...
async def predict_pockets(req: PredictPocketsRequest):
    try:
        return AIService.predict_pockets(req.structure_path)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def ai_decision(req: AIDecisionRequest):
    ...
    try:
        return AIService.ai_make_decision(
...
async def select_compound(req: SelectCompoundRequest):
    ...
    try:
        return AIService.select_best_compound(
            req.smiles_list,
            req.disease,
            req.protein,
            req.pocket_center,
...
async def molecule_image(req: MoleculeImageRequest):
    ...
    try:
        return AIService.generate_molecule_image(req.smiles)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def docking_image(req: DockingImageRequest):
    ...
    try:
        return AIService.generate_docking_image(
...
async def complete_workflow(req: CompleteWorkflowRequest):
    ...
    try:
        return AIService.complete_workflow(
...
async def decision_explanations():
    try:
        return AIService.get_decision_explanations()
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
