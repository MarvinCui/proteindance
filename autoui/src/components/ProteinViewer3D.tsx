...
const ProteinViewer3D: React.FC<Props> = ({ 
  ...
  // 调试结构文件加载问题
  const tryDebugStructure = async (structurePath: string) => {
    try {
      ...
      const debugUrl = `${API_BASE}/api/structure-debug/${encodedPath}`
      
      console.log('🔍 调试结构文件加载:', debugUrl)
      const response = await fetch(debugUrl)
      
      if (response.ok) {
        const debugInfo = await response.json()
        console.log('🛠️  结构文件调试信息:', debugInfo)
      } else {
        console.warn('⚠️  无法获取调试信息:', response.status)
      }
    } catch (err) {
      console.warn('⚠️  调试请求失败:', err)
    }
  }
  ...
  useEffect(() => {
    ...
    const loadStructure = async () => {
      try {
        ...
        // 获取结构数据 - 使用动态配置
        const { getApiBaseUrl } = await import('../config/api-config')
        const API_BASE = getApiBaseUrl().replace('/api', '') // 移除/api后缀，因为我们会手动添加
        // 确保路径正确编码，避免双斜杠问题
        const encodedPath = encodeURIComponent(structurePath)
        const structureUrl = `${API_BASE}/api/structure/${encodedPath}`
        
        console.log('🔗 加载结构文件:', structureUrl)
        const response = await fetch(structureUrl)
        
        if (!response.ok) {
          console.error('❌ 结构文件请求失败:', {
            url: structureUrl,
          ...
          if (response.status === 404) {
            throw new Error(`结构文件未找到: ${structurePath}。请确保文件已正确下载。`)
          }
        ...
        console.log('📄 收到结构数据，长度:', pdbData.length)
        
        if (!pdbData || pdbData.trim().length === 0) {
          console.error('❌ 接收到空的结构数据')
          throw new Error('接收到空的结构数据。请检查后端结构文件服务。')
        }
        ...
        // 提供更详细的错误信息
        let errorMessage = '加载结构失败'
        if (err instanceof Error) {
          if (err.message.includes('Failed to fetch')) {
            errorMessage = '网络连接失败：无法从后端获取结构文件。请检查后端服务是否正常运行。'
          } else if (err.message.includes('404')) {
            errorMessage = `结构文件未找到：${structurePath}。请确保蛋白质结构已正确下载。`
          } else if (err.message.includes('empty')) {
            errorMessage = '结构文件为空。请尝试重新下载结构文件。'
          } else {
            errorMessage = `加载结构失败: ${err.message}`
          }
        }
        ...
        // 尝试使用调试端点获取更多信息
        if (structurePath) {
          tryDebugStructure(structurePath)
        }
      }
    }
    ...
    for (let i = 0; i < smilesSubset.length; i++) {
      ...
      if (molData) {
        try {
          ...
          model.translate(pocketCenter[0] + offsetX, pocketCenter[1] + offsetY, pocketCenter[2] + offsetZ)
  ...
  const addOptimizedCompound = async (viewerInstance: any) => {
    ...
    if (molData) {
      try {
        ...
        // 优化化合物放在口袋中心
        model.translate(pocketCenter[0], pocketCenter[1], pocketCenter[2])
